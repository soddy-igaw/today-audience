...
## 에세이 3막 구조 (반드시 이 흐름을 따르세요)

### 1막: 감정/상황 — 왜 지금인가
- 가상 페르소나의 구체적 상황과 감정으로 시작
- 시장 배경을 2~3문단으로 설명
- 독자가 "아, 이런 사람 있지"라고 공감하게

### 2막: 타겟 정의 — 이 사람은 누구인가
- 행동 시그널 3단계로 타겟을 명확하게 정의
- DMP로 추적 가능한 시그널만 사용
- "이런 앱을 이런 패턴으로 쓰는 사람"을 구체적으로
- 구경꾼 vs 진짜 타겟을 비교해서 차이를 보여주기

### 3막: 전환 — 어떤 소비/행동으로 이어지는가
- 이 사람이 다음에 할 행동을 예측
- 광고주별 활용법 (어떤 광고를 언제 보여줘야 하는지)
- 오디언스 카드: 이름, 추정 모수, 핵심 시그널
